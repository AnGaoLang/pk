	...
	<view class="page-top">
		<view class="info-list">
			...
			<view class="info-item d-flex-jsb">
				...
				<view class="info-r d-flex">
					<input type="text" v-model="submit.pk_price" placeholder="PK金额不得低于10元">
					<image src="../../static/you.png" mode="widthFix"></image>
				</view>
			</view>
			<view class="info-item d-flex-jsb">
			...
			<view class="info-item d-flex-jsb">
				...
				<view class="info-r d-flex">
					<view class="pk-type d-flex">
						<view class="d-flex" @click="setFormat(i)" v-for="(t,i) in pk_format" :key="i">
							<text>{{t}}</text>
							<image v-if="submit.pk_format_id !== i" src="../../static/check.png" mode="widthFix"></image>
							<image v-if="submit.pk_format_id === i" src="../../static/checked.png" mode="widthFix"></image>
						</view>
						<!-- <view class="d-flex" @click="setFormat(2)">
							<text>3v3</text>
							<image v-if="submit.pk_format_id !== 2" src="../../static/check.png" mode="widthFix"></image>
							<image v-if="submit.pk_format_id === 2" src="../../static/checked.png" mode="widthFix"></image>
						</view> -->
					</view>
					<image src="../../static/you.png" mode="widthFix"></image>
				</view>
			...
			<view class="game-info">
				...
				<view class="pk" @click="showLoadingDialog">确认发布</view>
			</view>
		</view>
		<uni-footers></uni-footers>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getTuijian() {
				...
				this.$utils.request('api/pk/index', {}, res => {
					if (res.code == 200) {
						that.pk_explain = res.data.pk_explain;
						that.pk_format = res.data.pk_format;
