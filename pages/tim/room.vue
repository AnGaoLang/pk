<template>
	<!-- <view :style="`position: relative;height: 100%;overflow: hidden;margin-top: 40px;`"> -->
	<view>
				...
				<view class="row" v-for="(item,index) in msgList" :key="index" :id="item.ID">
					...
					<block>
						...
						<view class="other" v-else>
							...
							<view class="right">
								<view class="username">
									<view class="name">{{toUserInfo && toUserInfo.user}}</view>
			...
			getMsgList(){
				// 历史消息列表
					...
					promise.then((res)=> {
					  ...
					  this.scrollToView = res.data.messageList[0] && res.data.messageList[res.data.messageList.length-1].ID
